          ...
          <div className="bg-gradient-to-br from-[#C8A24A]/8 to-gray-900/50 border border-[#C8A24A]/20 rounded-2xl p-8 sm:p-12">
            <div className="grid lg:grid-cols-2 gap-12 items-center">
              ...
              <div className="grid grid-cols-4 gap-3">
                {platforms.map(p => (
                  <div key={p.name}
                    className="aspect-square rounded-xl border border-white/8 flex flex-col items-center justify-center gap-1.5 p-2 transition-all hover:scale-105 hover:border-white/20 cursor-default"
                    style={{ background: '#000000' }}>
                    <div style={{ color: p.color }}>{p.icon}</div>
                    <span className="text-[10px] font-bold text-center leading-tight" style={{ color: p.color }}>{p.name}</span>
                  </div>
                ))}
              </div>
            </div>
