...
// Social media posting delegated to Ayrshare — no direct platform keys needed.
...
function PostComposerModal({
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const scheduleDate = scheduleType === 'schedule' ? new Date(scheduleDateStr).toISOString() : undefined;
...
function CalendarPanel({ userId, integrations }: { userId: string | null; integrations: PostizIntegration[] }) {
  ...
  const loadPosts = useCallback(async () => {
    ...
    try {
      ...
      const data = { posts: [] }; // Ayrshare: use ayrshare-scheduled function
