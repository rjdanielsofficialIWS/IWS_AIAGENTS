...
function UserMenu({
  ...
  return (
    <div ref={ref} className="relative">
      <button
        onClick={() => setOpen(v => !v)}
        className="flex items-center gap-2 p-1.5 rounded-xl border transition hover:bg-white/08"
        style={{ borderColor: open ? `${GOLD}40` : BORDER }}
      >
        {avatar
      ...
      {open && (
        <div
          className="absolute right-0 top-full mt-1.5 w-52 rounded-2xl border overflow-hidden z-50"
          style={{ background: '#1e1e1e', borderColor: BORDER, boxShadow: '0 20px 60px rgba(0,0,0,0.6)' }}
        >
          <div className="px-4 py-3 border-b" style={{ borderColor: BORDER }}>
            <p className="text-sm font-bold text-white truncate">{name}</p>
            {user?.email && <p className="text-xs text-white/30 truncate mt-0.5">{user.email}</p>}
          </div>
          <div className="p-1.5">
            <button
              onClick={() => { onSignOut(); setOpen(false); }}
              className="w-full flex items-center gap-2.5 px-3 py-2 rounded-xl text-sm font-medium transition hover:bg-red-500/15"
...
function TopBar({
  ...
  return (
    <div
      ...
      style={{ borderColor: BORDER }}
    >
      <h1 className="text-sm font-bold text-white/70">{VIEW_LABELS[view]}</h1>
      <div className="flex items-center gap-2">
        <button
          onClick={onOpenPostLog}
          className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-medium border transition hover:bg-white/08"
          style={{ borderColor: BORDER, color: 'rgba(255,255,255,0.4)' }}
        >
          <ClipboardList className="w-3.5 h-3.5" />
