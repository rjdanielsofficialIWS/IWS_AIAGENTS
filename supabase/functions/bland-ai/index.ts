...
serve(async (req) => {
  ...
  try {
    ...
    // Route: POST /bland-ai/enhance-prompt - Enhance prompt using OpenAI
    if (path === '/bland-ai/enhance-prompt' && req.method === 'POST') {
      const authHeader = req.headers.get('Authorization')
      if (!authHeader) {
        return new Response(
          JSON.stringify({ error: 'Missing authorization header' }),
      ...
      try {
        ...
        const openaiResponse = await fetch('https://api.openai.com/v1/chat/completions', {
          ...
          body: JSON.stringify({
            ...
            messages: [
              {
                role: 'system',
                content: 'You are an expert AI agent consultant. Your job is to take basic AI agent requirements and enhance them with specific, actionable details. Make the requirements more comprehensive, detailed, and implementable. Focus on specific tasks, workflows, scripts, decision trees, and measurable outcomes. Keep the enhanced version professional and well-structured.'
              },
              {
                role: 'user',
                content: `Please enhance and expand these AI agent requirements to be more detailed and comprehensive:\n\n"${text}"\n\nProvide specific details about tasks, workflows, scripts, decision-making processes, and success criteria.`
              }
            ],
            max_tokens: 1000,
            temperature: 0.7
        ...
        const enhancedText = openaiData.choices[0]?.message?.content || text
        ...
        return new Response(
          JSON.stringify({ error: 'Failed to enhance prompt', details: error.message }),
          { status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
        )
      }
    }
