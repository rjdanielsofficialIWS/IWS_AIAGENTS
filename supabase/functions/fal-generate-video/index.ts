...
Deno.serve(async(req)=>{
  ...
  try{
    ...
    if(rem<=0)return new Response(JSON.stringify({error:"limit_reached",feature:"video_seconds",message:"You have used all "+eff+"s of AI video this month.",used,limit:eff,plan}),{status:429,headers:{...cors,"Content-Type":"application/json"}});
    if(secs>rem)return new Response(JSON.stringify({error:"limit_reached",feature:"video_seconds",message:"You only have "+rem+"s remaining. This "+secs+"s video would exceed your limit.",used,limit:eff,remaining:rem,plan}),{status:429,headers:{...cors,"Content-Type":"application/json"}});
    ...
    const payload={prompt,image_url:imageUrl,duration:String(secs),aspect_ratio:aspectRatio||"16:9",negative_prompt:"blurry, low quality, watermark, text overlay, ugly, distorted",cfg_scale:0.5,generate_audio:true};
