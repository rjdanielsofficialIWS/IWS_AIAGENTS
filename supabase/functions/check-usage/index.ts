...
Deno.serve(async(req)=>{
  ...
  const limits=PLAN_LIMITS[plan]??{ai_captions_per_month:0,posts_per_month:0,platforms_allowed:0,video_seconds_per_month:0,repurpose_allowed:false,ai_ideas_allowed:false};
  ...
  const usage={ai_captions_used:u?.ai_analyses_used??0,posts_scheduled:u?.posts_scheduled??0,video_seconds_used:u?.video_seconds_used??0,video_seconds_bonus:u?.video_seconds_bonus??0,caption_credits_bonus:u?.caption_credits_bonus??0};
  const effectiveLimits={...limits,video_seconds_per_month:limits.video_seconds_per_month===-1?-1:limits.video_seconds_per_month+usage.video_seconds_bonus,ai_captions_per_month:limits.ai_captions_per_month===-1?-1:limits.ai_captions_per_month+usage.caption_credits_bonus};
  return new Response(JSON.stringify({plan,limits:effectiveLimits,usage,period,at_caption_limit:effectiveLimits.ai_captions_per_month!==-1&&usage.ai_captions_used>=effectiveLimits.ai_captions_per_month,at_post_limit:effectiveLimits.posts_per_month!==-1&&usage.posts_scheduled>=effectiveLimits.posts_per_month,at_video_limit:effectiveLimits.video_seconds_per_month!==-1&&usage.video_seconds_used>=effectiveLimits.video_seconds_per_month}),{headers:{...cors,"Content-Type":"application/json"}});
});
