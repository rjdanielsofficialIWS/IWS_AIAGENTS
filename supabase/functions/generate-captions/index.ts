...
async function callClaudeJson(sys:string,usr:string,_inputSchema:any,max=3000){const ctrl=new AbortController();const t=setTimeout(()=>ctrl.abort(),55000);let r:Response;try{r=await fetch("https://api.openai.com/v1/chat/completions",{method:"POST",headers:{"Content-Type":"application/json","Authorization":"Bearer "+OPENAI_API_KEY},body:JSON.stringify({model:"gpt-4.1",max_tokens:max,messages:[{role:"system",content:sys},{role:"user",content:usr}],response_format:{type:"json_object"}}),signal:ctrl.signal});}catch(e:any){clearTimeout(t);if(e?.name==="AbortError")throw new Error("AI generation timed out. Please try again.");throw e;}finally{clearTimeout(t);}if(!r.ok)throw new Error("OpenAI error: "+await r.text());const d=await r.json();try{return JSON.parse(d.choices?.[0]?.message?.content??"{}");} catch{throw new Error("Generation failed. Please try again.");}}
